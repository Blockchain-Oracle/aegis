...
| A-2 | "Sentinel" codename leftover (project name is Aegis) | architecture.md:369 (ADR-004), story-mcp-01:134 | Replace `sentinel_*` → `aegis_*` in tool-name discussion; ADR-004 prose update |
...
| A-13 | `mcp-02` invents `aegis_judges.splunklib_security_fallback` module | story-mcp-02 | Replace with direct `from splunklib.ai.security import detect_injection` |
...
| # | Issue | Files | Fix |
|---|---|---|---|
| B-1 | Foundation-Sec API contract mismatch — caller and provider disagree on signature | story-mw-04 + story-mcp-05 call `aegis_judges.foundation_sec.explain(verdict, text)`; EPIC-05 ships `FoundationSecExplainer.explain(ctx: VerdictContext)`. story-mcp-05 also invents `run_search` which doesn't exist | Update story-mw-04 + story-mcp-05 to use the contract from EPIC-05 (`FoundationSecExplainer.explain(VerdictContext)`); add a `VerdictContext` Pydantic model to `aegis_core` if not already present |
...
| `packages/aegis_*/pyproject.toml` (×4) + `__init__.py` stubs (×4) | cicd-01 NEW + skel-01 NEW | **skel-01 owns NEW** |
...
| `splunk_apps/aegis_app/default/app.conf` + README | cicd-05 NEW (minimal shell for AppInspect job) + app-01 NEW (real shell) | **app-01 owns NEW**; cicd-05 picks up real shell as a precondition |
...
| `story-core-05-otel-hec-exporter-config.md` (EPIC-03) | `packages/aegis_core/src/aegis_core/otel_hec_exporter.py` + config | OTel → Splunk HEC bridge missing; architecture promise unverified |
...
| `story-judges-06-defenseclaw-python-shim.md` (EPIC-04 or EPIC-08) | `packages/aegis_judges/src/aegis_judges/defenseclaw_backend.py` | 3 stories (mw-02, mw-05, mcp-03) import it; no story creates it |
...
| `aegis_full_stack` callable in `report.py` has no owner | minor | Fold into existing `story-eval-05` |
