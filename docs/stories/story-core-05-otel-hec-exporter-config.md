# Story — OpenTelemetry → Splunk HEC exporter for aegis_core
...
**I want to** import a single `configure_hec_exporter(hec_url, hec_token, *, index="main", sourcetype="cisco_ai_defense:aegis_verdict")` from `aegis_core.otel_hec_exporter`, call it once at startup, and have every subsequent `emit_verdict_event(...)` call ship to Splunk HEC as a JSON event under the `cisco_ai_defense:aegis_verdict` sourcetype
...
- `packages/aegis_core/src/aegis_core/otel_hec_exporter.py` — NEW — ~320 LOC. Implements a Splunk-HEC-shaped OTel exporter subclassing `opentelemetry.sdk.trace.export.SpanExporter` (or the OTel `LogExporter` if the GenAI events land as log records on the configured collector — pick exporter type to match where `emit_verdict_event` from story-core-02 emits the event). For each captured span/log event whose `name == "gen_ai.evaluation.result"`, builds the Splunk HEC envelope: `{"time": <epoch from event.timestamp>, "sourcetype": <configured sourcetype>, "source": "aegis-otel", "index": <configured index>, "host": socket.gethostname(), "event": {<all event.attributes flattened>}}`. POSTs batches to `${hec_url}/services/collector/event` with header `Authorization: Splunk ${hec_token}` via `httpx.AsyncClient` (per ADR-stack `httpx` not `requests`). Wraps in `tenacity` retry — exponential backoff on 5xx + `httpx.TransportError`, max 3 attempts, give-up logs WARN via structlog. Public surface: `class SplunkHECExporter(SpanExporter)`, `def configure_hec_exporter(hec_url: str, hec_token: str, *, index: str = "main", sourcetype: str = "cisco_ai_defense:aegis_verdict", batch_size: int = 50, flush_interval_s: float = 5.0) -> None` (installs a `BatchSpanProcessor`-wrapped exporter on the global tracer provider), `def shutdown_hec_exporter() -> None` (graceful flush + close). Reads env-var fallbacks: `AEGIS_SPLUNK_HEC_URL` / `AEGIS_SPLUNK_HEC_TOKEN` / `AEGIS_SPLUNK_INDEX` / `AEGIS_SPLUNK_HEC_SOURCETYPE`. mypy --strict clean. ≤ 400 LOC.
- `packages/aegis_core/src/aegis_core/__init__.py` — UPDATE — re-export `SplunkHECExporter`, `configure_hec_exporter`, `shutdown_hec_exporter`; update `__all__`.
- `packages/aegis_core/pyproject.toml` — UPDATE — add `httpx`, `tenacity` (already in workspace; declare here for clarity).
- `packages/aegis_core/tests/test_otel_hec_exporter.py` — NEW — ≥ 14 behavioral tests using `respx` to mock HEC endpoints: exporter starts and ships a single event (Splunk envelope shape verified); the four required envelope keys (`time`, `sourcetype`, `source`, `event`) appear in every POST body; default `sourcetype` is `"cisco_ai_defense:aegis_verdict"`; `Authorization: Splunk <token>` header set verbatim; `--index` flows through to the envelope; batch flush triggers at `batch_size` events; batch flush triggers at `flush_interval_s` timeout; 5xx retry path retries up to 3 times then logs WARN; 4xx is non-retryable and logs ERROR; env-var fallback for missing kwargs; `shutdown_hec_exporter` drains pending events before returning; concurrent `emit_verdict_event` from multiple coroutines all land; only `gen_ai.evaluation.result` events ship (other span events are dropped at the exporter); no real network call escapes (respx asserts).
- `packages/aegis_core/tests/conftest.py` — UPDATE (or NEW if not present from core-02) — adds `respx_mock` fixture configured to intercept `*/services/collector/event`; provides a `splunk_hec_envelope` Pydantic helper for assertion shorthand.

The coding agent must NOT modify files outside this map without re-checking `CLAUDE.md`. In particular: **do not** modify `aegis_core/otel.py` from story-core-02 (this story consumes the events it emits, doesn't reshape them); **do not** add `requests` (banned per architecture); **do not** write the exporter as a generic OTel Splunk shim (it's Aegis-specific — name-filtered to `gen_ai.evaluation.result`).
...
```
Given packages/aegis_core/src/aegis_core/otel_hec_exporter.py exists
When  `uv run python -c "from aegis_core import configure_hec_exporter, shutdown_hec_exporter, SplunkHECExporter; print('ok')"` runs
Then  exit code is 0
...
Then  the default value is exactly "cisco_ai_defense:aegis_verdict"

Given a POST is captured
When  the `Authorization` header is inspected
Then  the value matches the regex `^Splunk [A-Za-z0-9_-]+$`

Given configure_hec_exporter(index="aegis_demo", sourcetype="cisco_ai_defense:aegis_verdict")
When  a verdict is emitted and the POST is captured
Then  the POST body has `index == "aegis_demo"`
And   `sourcetype == "cisco_ai_defense:aegis_verdict"`
...
Given AEGIS_SPLUNK_HEC_URL and AEGIS_SPLUNK_HEC_TOKEN are set in the env and configure_hec_exporter() is called with no kwargs
...
When  `uv run pytest packages/aegis_core/tests/test_otel_hec_exporter.py -v` runs
Then  ≥ 14 tests pass and 0 fail

Given mypy strict mode is active
When  `uv run mypy --strict packages/aegis_core/src/aegis_core/otel_hec_exporter.py` runs
Then  exit code is 0

Given ruff is run
When  `uv run ruff check packages/aegis_core/src/aegis_core/otel_hec_exporter.py` runs
Then  exit code is 0

Given the 400-LOC rule
When  `wc -l packages/aegis_core/src/aegis_core/otel_hec_exporter.py` runs
Then  the line count is ≤ 400

Given the §14 grep is run on the changed source (excluding tests)
When  `grep -rE "(mock|fake|dummy|hardcoded|simulated)" packages/aegis_core/src/aegis_core/otel_hec_exporter.py` runs
...
# 1. Imports resolve
uv run python -c "from aegis_core import configure_hec_exporter, shutdown_hec_exporter, SplunkHECExporter; print('ok')"

# 2. Tests pass (≥ 14)
uv run pytest packages/aegis_core/tests/test_otel_hec_exporter.py -v 2>&1 | tee /tmp/pytest.out
[ "$(grep -cE 'PASSED' /tmp/pytest.out)" -ge 14 ]

# 3. Strict typecheck
uv run mypy --strict packages/aegis_core/src/aegis_core/otel_hec_exporter.py

# 4. Lint
uv run ruff check packages/aegis_core/src/aegis_core/otel_hec_exporter.py

# 5. 400-LOC cap
[ "$(wc -l < packages/aegis_core/src/aegis_core/otel_hec_exporter.py)" -le 400 ]

# 6. §14 clean (production code path)
! grep -E "(mock|fake|dummy|hardcoded|simulated)" packages/aegis_core/src/aegis_core/otel_hec_exporter.py

# 7. Live HEC round-trip (gated on env vars per docs/architecture.md Hard Rule 6)
if [ -n "${AEGIS_SPLUNK_HEC_TOKEN:-}" ] && [ -n "${AEGIS_SPLUNK_HEC_URL:-}" ]; then
  uv run python - <<'PY'
import asyncio, uuid
from datetime import datetime, timezone
from aegis_core import configure_hec_exporter, shutdown_hec_exporter
from aegis_core.otel import emit_verdict_event
from aegis_core.verdict import Verdict, VerdictLabel, Severity
...
- **Per `../../../context/10-standards/02-otel-genai-semantic-conventions.md`**, the `gen_ai.evaluation.result` event is the spec slot Aegis emits. This exporter filters on `event.name == "gen_ai.evaluation.result"` and skips everything else — Aegis is not a general-purpose OTel-to-Splunk shim. If a future use case needs the generic shim, that's a separate package (and likely an upstream OTel exporter — do not invent here).
- **Per ADR-005 in `docs/architecture.md`**, the default sourcetype is `cisco_ai_defense:aegis_verdict` — load-bearing for colocation with Cisco Security Cloud (app 7404, 55K installs). Do not change the default.
- **Per `docs/architecture.md` § "Stack (locked)"**, use `httpx.AsyncClient` not `requests`, use `tenacity` for retry not hand-rolled backoff. Per Hard Rule 7, the exporter MUST default `verify=True` on TLS; if a dev needs self-signed certs they set `AEGIS_DEV_INSECURE_TLS=1` and the exporter logs a WARN at startup (do NOT silently `verify=False`).
- **Per `docs/architecture.md` Hard Rule 6 ("No real Splunk credentials in code or fixtures")**: tests use `respx` to mock the HEC endpoint. The live round-trip is gated on `AEGIS_SPLUNK_HEC_TOKEN`. Do not commit any token. Use `respx.MockRouter` to assert the exact Authorization header shape without ever issuing a real request.
...
- **Per `docs/architecture.md` § "Banned patterns"**, no `print()` for logs — use `structlog` (configured in story-core-04 `aegis_core.log`). Retry give-up and 4xx errors log via `structlog`.
...
- **Surface 4 (Splunk app) consumes these events** — the `event` payload's keys MUST match what `story-app-02`'s `FIELDALIAS-*` lines expect: `gen_ai.evaluation.score.label`, `gen_ai.evaluation.score.value`, `gen_ai.evaluation.explanation`, `aegis.surface`, `aegis.rules` (array of strings), `aegis.trace_id`, optionally `mcp.method.name` + `mcp.session.id`. Verify the post-flush envelope shape against `story-app-02`'s acceptance criteria.
- **mypy --strict requires explicit types on all `kwargs` and return values**. `dict[str, Any]` is the only place `Any` is allowed (HEC event payload is intentionally schema-flexible — but Aegis emits Verdict-shaped events only). Cite this carve-out inline.
