...
**Depends on:** story-app-02-props-transforms-for-aegis-verdict-sourcetype, story-eval-01-synthetic-data-generator-dns-guard-pattern
**Estimate:** ~2h
**Status:** PENDING
...
**I want to** run a single `python Synthetic-Data/scripts/emit_sample_verdict.py --count 500 --hec-url ${AEGIS_SPLUNK_HEC_URL} --hec-token ${AEGIS_SPLUNK_HEC_TOKEN}` command and have ≥ 500 realistic Aegis `Verdict` events appear in Splunk under `sourcetype="cisco_ai_defense:aegis_verdict"` within 30 seconds
...
When  every emitted event's `event` field is parsed as JSON and validated against `aegis_core.verdict.Verdict`
Then  zero validation errors are raised

Given the dry-run output is captured
When  every event's `sourcetype` is inspected
Then  every value equals exactly "cisco_ai_defense:aegis_verdict"
...
When  the set of `aegis.surface` attribute values across all events is collected
...
assert sourcetypes == {"cisco_ai_defense:aegis_verdict"}, sourcetypes
...
if [ -n "${AEGIS_SPLUNK_HEC_TOKEN:-}" ] && [ -n "${AEGIS_SPLUNK_HEC_URL:-}" ]; then
  uv run python Synthetic-Data/scripts/emit_sample_verdict.py \
    --count 50 \
    --hec-url  "${AEGIS_SPLUNK_HEC_URL}" \
    --hec-token "${AEGIS_SPLUNK_HEC_TOKEN}"
fi
...
- **Per ADR-005 in `docs/architecture.md`**, sourcetype `cisco_ai_defense:aegis_verdict` is load-bearing — it colocates with Cisco Security Cloud (app 7404 v3.6.6, 55K installs) so SOC analysts get unified search. Do NOT change the sourcetype string. The `props.conf` stanza from `story-app-02` parses on this exact value.
- **Per `docs/architecture.md` § "OTel emission shape"**, each event posted to HEC is `{"time": <epoch>, "sourcetype": "cisco_ai_defense:aegis_verdict", "source": "aegis-synthetic", "index": "<index>", "event": {<top-level Verdict JSON> + dotted OTel attrs}}`. Splunk HEC strips the outer envelope and the props.conf `INDEXED_EXTRACTIONS = json` lifts the `event` payload into fields.
...
- **Per `docs/architecture.md` Hard Rule 6 ("No real Splunk credentials in code or fixtures")**: tests use `--dry-run` + respx; the live HEC round-trip (shell block 9) only runs if `AEGIS_SPLUNK_HEC_TOKEN` is set. Do not bake any token into the script or tests.
- **§14 carve-out**: this script's purpose IS to synthesize fixture events for demos. Annotate the module docstring with `# §14 CARVE-OUT: synthetic fixture generator for demo + vision-loop. Not a production telemetry path.` and document the carve-out in `Synthetic-Data/scripts/README.md`. The §14 grep on production paths (`packages/aegis_*/src/`) does NOT cover `Synthetic-Data/`; see `docs/architecture.md` § "submission checklist gates".
- **HEC retry path** uses `tenacity` (already pinned via `aegis_judges` per EPIC-04) — retry on HTTP 5xx / connection errors with exponential backoff capped at 3 attempts; on final failure return exit 2.
