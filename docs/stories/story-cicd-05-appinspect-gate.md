...
- `splunk_apps/aegis_app/README` — PRECONDITION (owned NEW by story-app-01) — the real Splunkbase-required README ships in app-01. This story assumes it exists.
- `splunk_apps/aegis_app/default/app.conf` — PRECONDITION (owned NEW by story-app-01) — the real `app.conf` with all five stanzas ships in app-01. This story assumes it exists. (Prior draft of cicd-05 used to create a minimal shell here; per audit synthesis Block C the duplicate is removed.)
