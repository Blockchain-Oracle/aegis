...
- `packages/aegis_core/pyproject.toml` — PRECONDITION (owned NEW by story-skel-01)
- `packages/aegis_judges/pyproject.toml` — PRECONDITION (owned NEW by story-skel-01)
- `packages/aegis_mw/pyproject.toml` — PRECONDITION (owned NEW by story-skel-01)
- `packages/aegis_mcp/pyproject.toml` — PRECONDITION (owned NEW by story-skel-01)
- `packages/aegis_core/src/aegis_core/__init__.py` — PRECONDITION (owned NEW by story-skel-01)
- `packages/aegis_judges/src/aegis_judges/__init__.py` — PRECONDITION (owned NEW by story-skel-01)
- `packages/aegis_mw/src/aegis_mw/__init__.py` — PRECONDITION (owned NEW by story-skel-01)
- `packages/aegis_mcp/src/aegis_mcp/__init__.py` — PRECONDITION (owned NEW by story-skel-01)
...
- **OWNERSHIP NOTE — file-map flip from audit synthesis Block C.** All uv-workspace artifacts (`pyproject.toml` root, `uv.lock`, `.python-version`, every `packages/aegis_*/pyproject.toml`, every `packages/aegis_*/src/aegis_*/__init__.py`, `eval/pyproject.toml`) are owned NEW by story-skel-01. cicd-01 depends_on skel-01 and assumes those artifacts exist; the only NEW file in this story is `.github/workflows/ci.yml`. The BDD assertions confirm wheels build, but the package shells themselves are skel-01's deliverable.
