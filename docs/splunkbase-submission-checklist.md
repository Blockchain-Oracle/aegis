# Splunkbase Submission Checklist — Aegis

This checklist gates the Aegis app's Splunkbase submission. Every item
...
video references "install via dist/aegis_app-X.Y.Z.tgz", so the
...
- [ ] **1. AppInspect passes (zero unsuppressed error-severity findings)**
  ...
  - Verify: `bash splunk_apps/aegis_app/scripts/run_appinspect.sh`
...
- [ ] **2. `META-INF/manifest.json` is present and version matches `default/app.conf`**
  ...
  - Verify: `bash scripts/verify_splunkbase_artifact.sh dist/aegis_app-1.0.0.tgz`
  - Expect: exit 0; output includes "OK: ... passed all checks"
- [ ] **3. Extension-less `README` at app root**
  - Owned by: story-app-01 (skeleton)
  - Verify: `test -f splunk_apps/aegis_app/README`
  - Expect: exit 0
- [ ] **4. `LICENSE` at app root contains Apache-2.0 text**
  - Owned by: this story (app-12)
  - Verify: `head -5 splunk_apps/aegis_app/LICENSE | grep -E 'Apache License|Version 2.0'`
  - Expect: both substrings match
- [ ] **5. Icons present at the AppInspect-required sizes**
  - Owned by: story-app-09
  - Verify: `ls splunk_apps/aegis_app/static/appIcon{,_2x,Alt,Alt_2x}.png | wc -l`
  - Expect: `4`
- [ ] **6. Navigation XML present**
  - Owned by: story-app-02 (or app-01)
  - Verify: `test -f splunk_apps/aegis_app/default/data/ui/nav/default.xml`
  - Expect: exit 0
...
- [ ] **8. Demo video URL referenced in the README**
  ...
  - Verify: `grep -E 'youtu\.be|youtube\.com|loom\.com' splunk_apps/aegis_app/README`
  - Expect: at least one demo video link
- [ ] **9. Evaluation results table referenced in the README**
  - Owned by: EPIC-10 + EPIC-11
  - Verify: `grep -i "eval results" splunk_apps/aegis_app/README`
...
- [ ] **10. Supported Splunk versions match the Splunk Cloud demo target**
  ...
  - Verify: `uv run python -c "import json; m=json.load(open('splunk_apps/aegis_app/META-INF/manifest.json')); assert m['platformRequirements']['splunk']['Cloud'].startswith('10.4')"`
  - Expect: exit 0
...
| Field | Value |
|---|---|
| Path | `dist/aegis_app-1.0.0.tgz` |
| Source of truth for version | `splunk_apps/aegis_app/default/app.conf` |
