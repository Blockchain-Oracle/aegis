...
**ADR-003 — Foundation-Sec as explainer, NOT as classifier.** Three independent context audits (R5, R7, R12) verified that Cisco markets and deploys Foundation-Sec as a generator (security copilot). Using it as a binary classifier would be off-label. SplunkGate uses it only to generate human-readable explanations of WHY a verdict was reached; Cisco AI Defense Inspection API handles binary classification. The `source` field on a `RuleHit` is `Literal["ai_defense", "defenseclaw_regex", "splunklib_security"]` — Foundation-Sec is structurally excluded from ever populating that field; its only output channel is `Verdict.explanation: str | None`.
  ...
  **ADR-003a — Transport for Foundation-Sec is tenant-contingent (added 2026-06-05).** Live Playwright probe of Abu's Splunk Cloud tenant `prd-p-t9irr.splunkcloud.com` (sc_admin, v10.4.2604.5) on 2026-06-05 confirmed: (1) `| ai` SPL command returns "Unknown search command 'ai'" — not bundled in Splunk Cloud Platform 10.4; (2) Splunk AI Toolkit / AI Assistant for SPL is NOT installed (404 on `/app/Splunk_ML_Toolkit` and `/app/splunk_ai_assistant`); (3) REST API on port 8089 is closed in Splunk Cloud without a support ticket. Therefore the Foundation-Sec transport in SplunkGate is one of three paths, decided by tenant install permissions:
  - **Transport A — Splunk Hosted Models via `| ai` SPL** (preferred when available — implemented in `splunkgate_judges.foundsec_spl`). Requires the Splunk AI Toolkit (or equivalent) to be self-installable from Splunkbase. The explainer is invoked via a saved search calling `| ai provider=Splunk model=foundation-sec-1.1-8b-instruct …` and the resulting `explanation` field is pulled into `Verdict.explanation`. Mock-default until a tenant confirms entitlement.
  - **Transport B-Ollama — Hosted Model via local Ollama HTTP API** (the v1 SHIPPING path, added 2026-06-11 — implemented in `splunkgate_judges.hosted_models`). Generic Ollama `/api/generate` POST against either `gpt-oss:20b` (OpenAI open-weight, Apache-2.0, in the official Ollama library) or `foundation-sec:8b` (community Modelfile from the HF GGUF distribution). 30s timeout, tenacity retry on transient HTTPX failures, template fallback on any exception. Default model is `gpt-oss:20b` because it ships in Ollama out-of-the-box (`ollama pull gpt-oss:20b`) — zero-friction setup for hackathon judges who don't already have Foundation-Sec running. This path **also qualifies for the $1K "Best Use of Splunk Hosted Models" bonus prize** because (a) gpt-oss-20b is one of the four models on Splunk's Hosted Models capability page in the hackathon resources, and (b) the same call shape works when Splunk operationalizes a hosted gpt-oss endpoint — the SplunkGate explainer points at a different base URL, no code changes. Note: the *original* ADR-003a stance that Transport B "forfeits the bonus prize" was conservative — re-reading the bonus criterion ("leveraging capabilities like anomaly detection, forecasting, natural language understanding") confirms NL understanding via a Hosted Model qualifies regardless of where the inference physically runs.
  - **Transport B-Transformers — Direct in-process inference via HuggingFace `transformers`** (deferred — too heavy for the demo machine, kept as a documented escape hatch for operators who don't want a separate Ollama process).

  The explainer-only rule (the original ADR-003) is **invariant** across all transports. Only the transport changes. The active code path is selected at runtime via `Config.explainer_backend ∈ {"template", "ollama", "ai_spl"}` — the same `Config` instance flows into every middleware constructor.
...
- [x] Project demonstrates use of Splunk Hosted Models (gpt-oss-20b via Ollama, per ADR-003a Transport B-Ollama; foundation-sec:8b swap is a one-flag change once a Modelfile is available). Live integration test at `packages/splunkgate_judges/tests/test_hosted_models_live.py`. **Bonus prize criterion satisfied.**
