 ...
 * verdict_inspector on chart/heatmap/row click.
 *
 ...
 * cancelled-flag closure. Auto-refresh every 30s by default.
 *
 * Live-tick: when the BLOCKED KPI value increases, the tile gets a
 * 220ms pulse (the only motion in the product). Respects
...
(function () {
    ...
    function renderShell(root) {
        ...
        root.innerHTML = [
            ...
            '<p class="ar-panel-desc">Stacked verdict counts per hour over the selected window. ALLOW (paper green) sits at the bottom; BLOCK (vermillion) and MODIFY (amber) ride above. The eye should find HIGH-severity activity in under 5 seconds.</p>',
            ...
            '<p class="ar-panel-desc">Per-hour severity-weighted score for each of the 11 Cisco AI Defense rule names. Row order is verbatim from the Cisco Offer Description.</p>',
            '<div class="ar-heatmap" id="ar-heatmap-body"></div>',
            '</div>',
            '<div class="ar-panel">',
            '<h2>Top agents by BLOCKED count</h2>',
            '<p class="ar-panel-desc">Click a row to drill into the Verdict Inspector for that agent.</p>',
            '<div id="ar-top-agents-body"></div>',
            '</div>',
            '</section>',
            '<div class="ar-panel">',
            '<h2>MSJ scaling indicator (last 7 days)</h2>',
            '<p class="ar-panel-desc">Detection rate vs. in-context message count per agent — Many-Shot Jailbreaking probabilistic floor (Anthropic 2024). Higher detection_rate at high total_msgs = healthier agent.</p>',
    ...
    function kpi(id, label, value, suffix, extraClass) {
        return [
            '<div class="ar-kpi ' + extraClass + '" id="ar-' + id + '">',
            '<div class="ar-kpi-label">' + escapeHtml(label) + '</div>',
            '<div class="ar-kpi-value" id="ar-' + id + '-value">' + escapeHtml(value) + '</div>',
            '<div class="ar-kpi-suffix">' + escapeHtml(suffix) + '</div>',
            '</div>'
        ].join("");
    }
    ...
    function setKpi(id, value, suffix) {
        ...
        if (suffix) {
            var sufEl = valEl.parentElement && valEl.parentElement.querySelector(".ar-kpi-suffix");
    ...
    function tickKpi(id) {
        ...
        if (!el) { return; }
        // Toggle the class — animation forwards-only, so re-trigger by
        // removing and re-adding on the next frame.
        el.classList.remove("ar-tick");
        // Force reflow so the next class add re-triggers the keyframe.
        void el.offsetWidth;
        el.classList.add("ar-tick");
        setTimeout(function () { el.classList.remove("ar-tick"); }, 240);
    }
    ...
    function setKpiError(id, errMsg) {
        setKpi(id, "—", "load failed");
        // eslint-disable-next-line no-console
        if (typeof console !== "undefined" && console.warn) {
            console.warn("[splunkgate-agent-risk] KPI '" + id + "' load failed: " + errMsg);
        }
    }

    function renderKpiTotal(rows) {
        var v = (rows && rows[0] && rows[0].count) || "0";
        setKpi("kpi-total", formatNumber(v));
    }
    function renderKpiBlock(rows) {
        var v = (rows && rows[0] && rows[0].count) || "0";
        var num = parseInt(v, 10) || 0;
        if (state.lastBlockValue !== null && num > state.lastBlockValue) {
            tickKpi("kpi-block");
        }
        state.lastBlockValue = num;
        setKpi("kpi-block", formatNumber(v));
    }
    function renderKpiHigh(rows) {
        var v = (rows && rows[0] && rows[0].count) || "0";
        setKpi("kpi-high", formatNumber(v));
    }
    function renderKpiAgents(rows) {
        var v = (rows && rows[0] && rows[0].agents) || "0";
        setKpi("kpi-agents", formatNumber(v));
    }
    function renderKpiTokens(rows) {
        var v = (rows && rows[0] && rows[0].tokens_saved) || "0";
        setKpi("kpi-tokens", formatNumber(v));
    }

    /* Area chart — stacked SVG.
     * rows: [{ _time, ALLOW, BLOCK, MODIFY, REVIEW, ... }]
     */
    function renderArea(rows) {
        ...
        // Collect series keys deterministically.
        var seriesOrder = ["ALLOW", "MODIFY", "REVIEW", "BLOCK"];
        var present = {};
        ...
        var series = seriesOrder.filter(function (s) { return present[s]; });
        Object.keys(present).forEach(function (k) {
            if (series.indexOf(k) === -1) { series.push(k); }
        });

        // x: index → linear; y: stacked totals.
        var n = rows.length;
        var xStep = (width - pad.left - pad.right) / Math.max(1, n - 1);
        ...
        var yMax = Math.max(1, stacks.reduce(function (m, s) { return Math.max(m, s._total); }, 0));
        ...
        // Build polygon points for each series (stack from bottom up).
        var polygons = series.map(function (s) {
    ...
    /* Heatmap — 11 fixed rule rows × hour buckets.
     * rows: [{ _time, rule, score }]
     */
    function renderHeatmap(rows) {
        ...
        // Build hour set, sorted.
        var hourSet = {};
        rows.forEach(function (r) {
            var t = r._time || "";
            if (t.length >= 13) {
                hourSet[t.substring(0, 13) + ":00"] = true;
            } else {
                hourSet[t] = true;
            }
        });
        ...
        // Build map: rule × hourLabel → score
        var map = {};
        rows.forEach(function (r) {
            var t = r._time || "";
        ...
        // Compute intensity buckets (0-5) per cell.
        var allScores = Object.keys(map).map(function (k) { return map[k]; });
        var maxScore = allScores.reduce(function (m, v) { return Math.max(m, v); }, 1);
        ...
        // Body rows — verbatim Cisco rule order.
        var bodyRows = CISCO_AI_DEFENSE_RULES.map(function (rule) {
            var cells = '<th class="ar-rule-label">' + escapeHtml(rule) + "</th>";
            hours.forEach(function (hh) {
                var key = rule + "||" + hh;
                var score = map[key] || 0;
                cells += '<td class="ar-heatmap-cell" data-intensity="' + bucket(score) + '" title="' + escapeHtml(rule + " @ " + hh + " — score=" + score.toFixed(1)) + '"></td>';
            });
            return "<tr>" + cells + "</tr>";
        }).join("");
    ...
    function renderTopAgents(rows) {
        ...
        var trs = rows.map(function (r) {
            ...
            return (
                "<tr>" +
                '<td class="ar-mono"><a href="' + escapeHtml(url) + '">' + escapeHtml(aid) + "</a></td>" +
    ...
    function renderMsj(rows) {
        ...
        var line = "M " + points.map(function (p) { return xScale(p.x) + "," + yScale(p.y); }).join(" L ");
        var dots = points.map(function (p) {
            return '<circle class="ar-msj-dot" cx="' + xScale(p.x) + '" cy="' + yScale(p.y) + '" />';
        }).join("");
        // Axis: y = detection_rate (0–1), x = total_msgs (0–xMax).
        var yTicks = (
    ...
    function refreshAll() {
        ...
        runSearch("total", QUERIES.total, 1, e, l, renderKpiTotal, function (msg) { setKpiError("kpi-total", msg); });
        runSearch("block", QUERIES.block, 1, e, l, renderKpiBlock, function (msg) { setKpiError("kpi-block", msg); });
        runSearch("high", QUERIES.high, 1, e, l, renderKpiHigh, function (msg) { setKpiError("kpi-high", msg); });
        runSearch("agents", QUERIES.agents, 1, e, l, renderKpiAgents, function (msg) { setKpiError("kpi-agents", msg); });
        runSearch("tokens", QUERIES.tokens_saved, 1, e, l, renderKpiTokens, function (msg) { setKpiError("kpi-tokens", msg); });
        runSearch("ts", QUERIES.ts, 200, e, l, renderArea, function (msg) { setPanelError("ar-area-body", msg); });
        runSearch("heatmap", QUERIES.heatmap, 5000, e, l, renderHeatmap, function (msg) { setPanelError("ar-heatmap-body", msg); });
        runSearch("top_agents", QUERIES.top_agents, 10, e, l, renderTopAgents, function (msg) { setPanelError("ar-top-agents-body", msg); });
        // MSJ uses fixed 7-day window per the source spec.
        runSearch("msj", QUERIES.msj, 500, "-7d", "now", renderMsj, function (msg) { setPanelError("ar-msj-body", msg); });

        var gen = document.getElementById("ar-footer-generated");
        if (gen) { gen.textContent = "Last refresh: " + new Date().toLocaleTimeString(); }
        var ind = document.getElementById("ar-refresh-indicator");
        if (ind) { ind.textContent = state.refreshIntervalMs > 0 ? "live" : "manual"; }
    }
    ...
    function wireControls() {
        ...
        if (t) {
            t.addEventListener("change", function (e) {
                ...
                // Reset BLOCK live-tick baseline when the window changes —
                // otherwise the first refresh after a window change pulses
