...
A single UUID threads through Verdict, OTel event attrs, AegisError instances,
...
stories emit `aegis.trace_id` as an OTel event attr).
"""
...
_trace_id_var: ContextVar[UUID | None] = ContextVar("aegis_trace_id", default=None)


def new_trace_id() -> UUID:
    """Mint a fresh UUID for a logical Aegis request."""
    return uuid4()
