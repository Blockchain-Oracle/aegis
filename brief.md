...
The name "SplunkGate" refers to the mythological shield carried by Athena and Zeus — a literal protective surface. The brand glyph (already exists, in `splunk_apps/splunkgate_app/static/appIcon.png`) is a simplified pentagon-shield silhouette in Splunk blue (`#1A8FFF`) on dark navy (`#1A1C20`). It's a placeholder generated by Pillow — feel free to commission or generate a higher-fidelity version for the landing page; the shield-with-notch silhouette is the only required continuity.
